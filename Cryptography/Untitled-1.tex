...
\begin{frame}{DES(Data Encryption Standard)}
    ...
    \begin{figure}[b]
        ...
        \label{fig:暗号化の概要}
        \begin{tikzpicture}
            \tikzset{data/.style={draw,rounded corners,rectangle,text centered, minimum height = 1cm,text width=2cm}};
            \tikzset{Key/.style={fill=red!10,rounded corners,rectangle,text centered, minimum height = 1cm,text width=2cm}};
            \tikzset{Encryption/.style={draw,very thick,rectangle,text centered,minimum height=1cm,text width=2cm}};
...
\begin{frame}
    \begin{figure}[t]
        ...
        \begin{tikzpicture}
            \tikzset{data/.style={draw,rounded corners,rectangle,text centered, minimum height = 1cm,text width=2cm}};
            \tikzset{Key/.style={draw,rounded corners,rectangle,text centered, minimum height = 1cm,text width=2cm}};
            \tikzset{Encryption/.style={draw,very thick,rectangle,text centered,minimum height=1cm,text width=2cm}};
            \tikzset{XOR/.style={draw,thick,circle,text centered}};
            
            \node[data](left){\(a\)};
            \node[data,right=of left](right){\(b\)};
            \node[XOR,below=of left](XOR){XOR};
            \node[data,fill=gray!20,below=of XOR](left2){\(a'\)};
            
            \node[data,right=of left2](right2){\(b\)};
            \draw[-latex,thick](left)--(XOR);
            \draw[-latex,thick](XOR)--(left2);
            \draw[-latex,thick](right)--(right2);
