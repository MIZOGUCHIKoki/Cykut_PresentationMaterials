...
\begin{frame}
    \begin{figure}[t]
        ...
        \begin{tikzpicture}
            \node[empty](center){};
            \node[Key,below=0.5cm of center](Key){Sub Key};
            \node[Encryption,below=0.5cm of Key](Encryption){ラウンド関数\(f\)};
            \node[left=1cm of Encryption,scale=3](XOR){\(\oplus\)};
            \node[color=white,right=1cm of Encryption,scale=3](XOR2){$\oplus$};
            \node[data,above=1.5cm of XOR](a){\(a\)};
            \node[data,above=1.5cm of XOR2](b){\(b\)};
            \node[data,below=1.5cm of XOR2](b2){\(b\)};
            \node[data,below=1.5cm of XOR](a2){\(a'\)};
            \draw[-latex,very thick](a)--(XOR);
            \draw[-latex,very thick](XOR)--(a2);
            \draw[-latex,very thick](Key)--(Encryption);
            \draw[-latex,very thick](b)--(b2);
            \draw[-latex,very thick](b)|-(Encryption);
            \draw[-latex,very thick](Encryption)--(XOR);
        \end{tikzpicture}
    \end{figure}
\end{frame}
